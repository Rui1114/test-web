...
// ─── Region definitions ───────────────────────────────────────
// 省份顺序按逾期欠款项目数从多到少排列（4月底数据，共76个项目）
const REGIONS = {
  '北部区域': ['河北', '辽宁', '内蒙古', '北京', '天津', '吉林'],   // 25: 河北14,辽宁4,内蒙3,北京2,天津1,吉林1
  '华南区域': ['广东', '湖南', '福建', '广西', '湖北', '江西'],      // 12: 广东5,湖南2,其余1
  '中部区域': ['陕西', '山西', '河南'],                              // 11: 陕西6,山西3,河南2
  '东部区域': ['山东', '浙江'],                                      // 11: 山东11
  '贵州区域': ['贵州'],                                              // 9
  '西南区域': ['云南', '四川', '重庆'],                              // 8: 云南5,四川2,重庆1
};
...
// 逾期欠款数量：北部25 > 华南12 > 中部11 = 东部11 > 贵州9 > 西南8（共76个，4月底数据）
const REGION_SUMMARIES = {
  '北部区域': {
    tags: ['逾期25个·最多', '再融资主导'],
    summary: '本区域逾期欠款地区25个，全网最多。河北14个（秦皇岛、唐山、沧州、保定等），覆盖范围最广，已纳入自审自发试点，发债节奏明显提速，是最优先跟进省份；辽宁4个（沈阳、铁岭），特殊专项债占比超60%，清偿拖欠账款优先级高；内蒙古3个（呼和浩特），化债资金集中向清欠方向倾斜；北京2个（延庆、昌平）、天津1个（汉沽）、吉林1个（长春农安，已退出高风险名单，新增额度空间打开）。'
  },
  '华南区域': {
    tags: ['逾期12个', '发行规模最大'],
    summary: '本区域逾期欠款地区12个。广东5个（深圳宝安、韶关仁化/南雄、茂名、江门），新增专项债全国占比最高，清偿窗口最确定；湖南2个（永州新田、江永），清欠纳入省级督查激励体系，存量欠款清偿概率高；福建1个（莆田荔城）、广西1个（贵港）、湖北1个（宜昌伍家岗）、江西1个（赣州全南），湖北、江西相继纳入自审自发扩围，审批效率大幅提升；此外浙江嘉兴平湖1个由华南团队跟进。华南为整体发债最活跃区域。'
  },
  '中部区域': {
    tags: ['逾期11个', '省委强力部署'],
    summary: '本区域逾期欠款地区11个。陕西6个（咸阳5个：永寿、彬县、礼泉、渭城、大荔；西安蓝田1个），省委专题部署"加力清理拖欠企业账款"，力争隐性债务全面清零，政策落地力度全国领先；山西3个（太原晋源、尖草坪、清徐），以再融资为主，重点关注服务类欠款清理窗口；河南2个（开封兰考、洛阳），补充财力类清欠台账已录入171个项目。'
  },
  '东部区域': {
    tags: ['逾期11个', '集中山东'],
    summary: '本区域逾期欠款地区11个，全部集中于山东省（淄博4个：张店、周村、经开区、淄川；枣庄2个；德州2个：齐河、宁津；烟台龙口、泰安肥城、济宁鱼台各1个），覆盖6个地市。山东发债批次密集、规模全国前列，但化债资金占比偏低，以建设类专项债为主；浙江财政实力强，债务压力相对较小，化债资金主要来源于自审自发补充财力类资金。'
  },
  '贵州区域': {
    tags: ['逾期9个', '全国化债重点'],
    summary: '本区域逾期欠款地区9个，全部位于贵州省：贵阳5个（清镇、开阳、锦绣片区、修文、贵阳市区），毕节1个（大方），遵义1个（务川），铜仁1个（沿河），六盘水1个（盘州）。全国化债重点省份，新增专项债主要向化债方向倾斜，补充财力类资金优先用于清偿企业欠款。建议优先纳入全口径债务监测平台，争取下一批次资金拨付。'
  },
  '西南区域': {
    tags: ['逾期8个', '化债力度大'],
    summary: '本区域逾期欠款地区8个。云南5个（昆明五华区2个、西山区1个、楚雄禄丰1个、武定1个），大规模置换债持续落地，资金到账后30–60天为最佳跟进窗口；四川2个（资阳安岳、自贡），化债力度全国最强之一，省级承担5%配套减轻市县压力；重庆1个（巴南区），已纳入自审自发扩围，再融资释放大量财政现金流。'
  }
};
...
const SUBSIDY_REGION_SUMMARIES = {
  '北部区域': '逾期欠款25个（全网最多）：河北（14个逾期项目）自审自发试点获批后超长期特别国债和中央预算内投资申报节奏明显提速，是政策性资金最优先跟进省份；辽宁（4个）环保基础设施申报空间开放；内蒙古（3个）已获行业专项补贴支持，与154.5亿清欠资金叠加推进；北京（2个）、天津（1个）、吉林（1个）退出高风险名单后申报空间打开。2026年北部区域政策性资金整体活跃度持续提升。',
  '华南区域': '逾期欠款12个：广东（5个）超长期特别国债支持规模全国领先，环保基础设施申报条件最成熟；湖南（2个）清欠督查激励体系叠加补贴资金到位；福建、广西、湖北、江西（各1个）均已启动2026年中央预算内投资备案申报，广西湖南受行业专项补贴重点覆盖。华南为整体政策活跃度最高区域之一。',
  '中部区域': '逾期欠款11个：陕西（6个，省委专题部署力度全国领先）申报窗口最活跃，2026年超长期特别国债和中央预算内投资批次持续；山西（3个）2026年申报窗口开放，建议重点跟进中央预算内投资新一批次动态；河南（2个）超长期特别国债设备更新已覆盖郑州、洛阳等核心城市，中央预算内投资在171个清欠台账项目框架下持续下达。',
  '东部区域': '逾期欠款11个，全部集中在山东（11个，涉及淄博、枣庄、德州、烟台、泰安、济宁六地市）：东部区域财政实力较强，中央配套比例相对偏低；山东、浙江暂无政策性资金记录，但中央预算内投资和超长期特别国债渠道对两省保持开放，建议持续关注2026年扩围申报批次动态。',
  '贵州区域': '逾期欠款9个，全部位于贵州省（贵阳5个、毕节1个、遵义1个、铜仁1个、六盘水1个）：贵州是四大政策性资金渠道全覆盖、政策倾斜最强的省份。2025年超长期国债设备更新已落位约18亿，中央预算内投资、新型政策性金融工具、行业专项补贴均有专项安排；2026年继续扩围申报，是业务优先跟进区域。',
  '西南区域': '逾期欠款8个：云南（5个，主要集中昆明及楚雄）获新型政策性金融工具优先支持，2026年8000亿工具资金西南区域分配比例较高，资金到位后30–60天为最佳催款窗口；四川（2个）是超长期特别国债和中央预算内投资全国最活跃省份之一；重庆（1个）已纳入自审自发扩围，多批次资金有效支撑债务清偿。'
};
